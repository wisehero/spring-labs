...
JPA에서 가장 빈번하게 발생하는 성능 문제인 **N+1 쿼리 문제**를 직접 재현하고, JOIN FETCH, @EntityGraph, DTO Projection 등의 해결 방법을 실험한다.

N+1 문제란: 연관 엔티티를 조회할 때 **1번의 메인 쿼리** + **N번의 추가 쿼리**가 발생하여 데이터가 많을수록 성능이 급격히 저하되는 현상이다.
...
**아니다.** `FetchType.EAGER`는 N+1을 해결하지 않고, 오히려 더 나쁜 결과를 초래한다:
