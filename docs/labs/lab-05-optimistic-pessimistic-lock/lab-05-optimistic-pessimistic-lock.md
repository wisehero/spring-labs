...
**낙관적 락(Optimistic Lock)** 과 **비관적 락(Pessimistic Lock)** 을 Product 엔티티의 재고(stock) 관리 시나리오로 비교한다.

재고 차감, 좌석 예약, 포인트 사용 등 동시 수정이 빈번한 도메인에서 잘못된 동시성 제어는 **Lost Update**, **데이터 불일치**, **데드락**을 유발한다. 두 전략의 트레이드오프를 이해하고 상황에 맞게 선택해야 한다.
