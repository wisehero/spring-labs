...
`@Transactional`의 전파 옵션 중 **REQUIRED**(기본값)와 **REQUIRES_NEW**의 동작 차이를 9가지 실험으로 검증한다.

- **REQUIRED**: 기본값. 기존 트랜잭션이 있으면 참여, 없으면 새로 생성.
- **REQUIRES_NEW**: 항상 새 트랜잭션을 생성. 로깅, 감사, 부분 커밋이 필요한 경우 사용.

두 옵션의 차이를 정확히 이해하지 못하면 **UnexpectedRollbackException**, **커넥션 풀 고갈**, **데이터 불일치** 같은 운영 장애로 이어진다.
...
**핵심 차이**: REQUIRED는 트랜잭션을 "공유"하고, REQUIRES_NEW는 항상 "독립"적이다.
...
| 4-9 | 커넥션 풀 고갈 | POST | **connectionTimeout만큼 소요** |
...
**의미**: REQUIRED는 기존 트랜잭션이 있으면 자동으로 참여한다. begin/commit이 한 번만 발생한다.
...
**의미**: REQUIRED는 "있으면 참여, 없으면 생성"의 유연한 기본값이다.
...
**의미**: REQUIRES_NEW는 기존 트랜잭션을 일시 중단(suspend)하고 완전히 새로운 트랜잭션을 시작한다.
...
**왜 이런가?**: Inner가 REQUIRED로 같은 트랜잭션에 참여했으므로, Inner의 예외 시 Spring TX 인터셉터가 트랜잭션을 **rollback-only**로 마킹한다. Outer가 예외를 catch해도 이미 트랜잭션은 오염된 상태. 커밋 시도 시 Spring이 `UnexpectedRollbackException`을 던진다.

**실무 교훈**: REQUIRED 공유 트랜잭션에서 inner 예외를 catch하는 것은 "트랩"이다. 예외를 잡아도 트랜잭션은 이미 죽은 상태다.
...
**핵심**: Scenario C가 특히 위험하다. 예외 없이 `setRollbackOnly()`만 호출해도 Outer 커밋이 실패한다.
...
**의미**: REQUIRES_NEW는 실제로 **2개의 DB 커넥션을 동시에 점유**한다. Outer 커넥션은 suspend 상태로 반환되지 않는다.
...
**WARNING: 이 실험은 HikariCP connectionTimeout만큼 소요됩니다!**

**동작**: REQUIRES_NEW를 (실제 풀 사이즈 + 1)단계 재귀 중첩하여 풀 고갈을 유발. 풀 사이즈는 런타임에 `HikariDataSource.maximumPoolSize`에서 동적으로 읽습니다.

**예상 결과**: 풀 사이즈까지 성공, 풀 사이즈 + 1에서 커넥션 획득 대기 후 timeout
...
3. **inner 예외를 catch하는 코드를 의심할 것**: REQUIRED 공유 트랜잭션에서 inner 예외 catch는 트랩이다.
...
- [Lab 01: @Transactional 자기 호출 함정](../lab-01-transactional-self-invocation/lab-01-transactional-self-invocation.md) - AOP 프록시와 self-injection 패턴
- [Lab 02: @Transactional(readOnly=true) 실제 효과](../lab-02-transactional-readonly/lab-02-transactional-readonly.md)
- [Lab 03: Bulk Insert 성능 비교](../lab-03-bulk-insert-performance/lab-03-bulk-insert-performance.md) - self-injection 패턴 활용
