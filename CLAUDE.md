...
**핵심 원칙**: 이 프로젝트에서 Claude는 기본적으로 코드를 **직접 작성/수정하지 않는다**. 사용자가 직접 타이핑하여 학습하는 것이 목적이다.

### 기본 모드: 제안만 수행

- **금지 도구 (기본)**: `Edit`, `Write`, `NotebookEdit` — 소스 코드 파일(`.kt`, `.java`, `.gradle`, `.properties`, `.sql`, `.http` 등)에 대해 사용하지 않는다.
- **Claude의 역할**: 제안만 한다.
  - 변경할 파일 경로를 `file_path:line_number` 형식으로 명시
  - 변경 내용을 코드 블록으로 보여주고, **왜** 그렇게 변경해야 하는지 설명
  - 여러 대안이 있으면 트레이드오프와 함께 제시
- **사용자의 역할**: 제안을 보고 직접 파일을 편집한다. 이후 Claude에게 검토/설명/디버깅을 요청할 수 있다.
- **검토/분석/탐색은 자유롭게**: `Read`, `Grep`, `Glob`, `Bash`(읽기 전용) 등은 제한 없이 사용한다.

### 예외: 명시적 요청이 있으면 직접 작성 허용

사용자가 **명시적으로** 코드 작성을 요청한 경우에는 Claude가 직접 파일을 편집할 수 있다. 다음과 같은 표현이 명시적 요청에 해당한다:

- "직접 작성해줘", "직접 수정해줘", "코드 짜줘", "구현해줘"
- "파일 만들어줘", "~파일에 써줘"
- 기타 사용자가 분명히 편집을 지시하는 문장

**적용 범위**: 명시적으로 요청된 그 작업의 범위 내에서만 편집한다. 요청되지 않은 주변 파일까지 임의로 수정하지 않는다.

**의심스러우면 물어본다**: 요청이 "제안해줘"인지 "작성해줘"인지 불분명하면 먼저 **"직접 편집할까요, 제안만 드릴까요?"** 를 확인한다.

### 항상 허용되는 쓰기 작업

별도 확인 없이 편집 가능한 파일 (단, 여전히 변경 사항은 보고한다):

- 문서 파일(`docs/**/*.md`, `README.md`) — 문서 작성/수정 요청 시
- `.claude/**`, `CLAUDE.md` 등 메타/설정 파일
